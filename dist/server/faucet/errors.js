...
const formatError = (err) => {
    ...
    const notOnAlchemyWhitelist = message.includes('Unspecified origin not on whitelist');
    ...
    const notEnoughGas = message.includes('max fee per gas less than block base fee');
    const gasTooLowForNextBlock = message.includes('is too low for the next block');
    ...
    //   cannotEstimateGas,
    // })
    const callException = (err === null || err === void 0 ? void 0 : err.code) === 'CALL_EXCEPTION' && (err === null || err === void 0 ? void 0 : err.method)
        ? `error calling contract. check funds`
        : null;
    let error = callException !== null && callException !== void 0 ? callException : 'An unknown error occured. Please reach out to #support';
